...
Shader "FK/FKToon/Toon Advanced Dissolve" {
    Properties {
        ...
        _DissolveAmount ("DissolveAmount", Range(-0.1, 1.1)) = -0.1
        _Dissolve ("Dissolve", 2D) = "black" {}
        _DissolveRamp ("DissolveRamp", 2D) = "black" {}
        _Displace ("Displace", Float ) = 0
        _BurnAmount ("BurnAmount", Range(-0.1, 1.1)) = -0.1
...
////// Emissive:
                ...
                float4 _BurnTex_var = tex2D(_BurnTex,TRANSFORM_TEX(i.uv1, _BurnTex));
                float2 node_7560 = float2((1.0 - saturate(((((1.0 - _BurnAmount)*1.2+-0.6)+_BurnTex_var.r)*8.0+-4.0))),1.0);
                float4 node_1586 = tex2D(_DissolveRamp,node_7560);
                float3 emissive = ((_EmissionColor.rgb*_EmissionTex_var.rgb)+(_RimColor.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_RimPower)*_RimColor.a)+node_9381.rgb+node_1586.rgb);
                float node_6426 = 0.5;
