...
public class EnemyFast : Enemy 
{
	...
	public override void  GuardBehaviour()
	{
		...
		{
			// Check if nearest target is going to attack
			if(nearTarget.Combat.IsAttacking())
			{
				// Reset character movement can move state
				movement.CanMove = true;

				// Set move vector to near target backward
				move = (trans.position - nearTarget.Trans.position).normalized;
			...
			#if DEBUG_BUILD
				// Trace debug message
				Debug.Log("EnemyFast: the target " + nearTarget.gameObject.name + " is going to attack");
			#endif
			}
			else
			{
